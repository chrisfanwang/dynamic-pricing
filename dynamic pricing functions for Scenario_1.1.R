...
transfer_learning_pricing_se1 <- function(n_Q, X_target, D_P, C_r, kappa_tilde_r, d, gamma, theta_0, theta, tilde_theta, nu, C_I, some_revenue_function) {
  # Initialize
  t <- 1
  ...
  while (t <= n_Q) {
    ...
    f_xt_pt <- some_revenue_function(X_t, p_t,  theta_0, theta, tilde_theta)
    Y_t <- runif(1 , min =  f_xt_pt - nu, max =  f_xt_pt + nu)
    # Record results
    results$t[t] <- t
    results$p_t[t] <- p_t
...
ABE_algorithm_se1 <- function(T, X_target, d, theta_0, theta, tilde_theta, nu, revenue_function, M = 0.1) {
  ...
  for (t in 1:T) {
    ...
    if (k < K) {
      if (B$N < n_k(k)) {
        ...
        f_xt_pt <- revenue_function(X_t, p_t, theta_0, theta, tilde_theta)
        Y_t <- runif(1 , min =  f_xt_pt - nu, max =  f_xt_pt + nu)
...
ExUCB_algorithm_se1 <- function(n_Q, X_target, d, theta_0, theta, tilde_theta, nu, revenue_function) {
  ...
  for (j in 1:epi){
    ...
    for (i in 1:explore_l){
      ...
      er[idx] <- revenue_function(x1[idx, ], S[idx], theta_0, theta, tilde_theta)
      # Simulate observed revenue
      y[idx] <-runif(1 , min =   er[idx] - nu, max =   er[idx] + nu)
      r[idx] <- y[idx] / S[idx]
...
compute_n_re_P <- function(B, D_P) {
  ...
  if (is.null(D_P)){
    ...
    D_P_Xp <- as.matrix(cbind(D_P$X.1,D_P$X.2, D_P$p))
...
generate_p_samples <- function(N, tilde_p, tilde_r, kappa) {
  # Ensure tilde_p is within [0,1]
  if (tilde_p < 0 || tilde_p > 1) {
    stop("tilde_p must be between 0 and 1.")
  }
  
  # Ensure tilde_r is between 0 and 1
  if (tilde_r <= 0 || tilde_r > 1) {
    stop("tilde_r must be between 0 and 1 (excluding 0).")
  }
  ...
  # Compute the inner interval [a, b]
  a <- max(0, tilde_p - tilde_r / 2)
  b <- min(1, tilde_p + tilde_r / 2)
  ell_inner <- b - a  # Length of the inner interval
  
  # Compute the densities
  p_inner <- kappa
  p_outer <- (1 - kappa * ell_inner) / (1 - ell_inner)
  
  # Compute the probability masses
  P_inner <- p_inner * ell_inner
  P_outer <- 1 - P_inner  # Since total probability must be 1
  ...
  for (i in 1:N) {
    u <- runif(1)
    if (u <= P_inner) {
      # Sample from inner interval
      p_samples[i] <- runif(1, min = a, max = b)
    } else {
      # Sample from outer intervals
      L <- a       # Length of left outer interval [0, a)
      R <- 1 - b   # Length of right outer interval (b, 1]
      ell_outer <- L + R  # Total length of outer intervals
      
      v <- runif(1, min = 0, max = ell_outer)
      
      if (L > 0 && R > 0) {
        if (v < L) {
          p_samples[i] <- v  # Sample from [0, a)
        } else {
          p_samples[i] <- b + (v - L)  # Sample from (b, 1]
        }
      } else if (L > 0) {
        # Only left interval exists
        p_samples[i] <- v  # Sample from [0, a)
      } else if (R > 0) {
        # Only right interval exists
        p_samples[i] <- b + v  # Sample from (b, 1]
      } else {
        stop("No outer interval available for sampling.")
      }
    }
  }
...
revenue_function <-  function(x, p, theta_0, theta, tilde_theta) {
  # x: a vector of length d
  # p: a scalar in [0, 1]
  # theta_0: a scalar
  # theta: a vector of length d
  # tilde_theta: a scalar
  
  # Ensure x and theta are of the same dimension
  if (length(x) != length(theta)) {
    stop("x and theta must have the same length")
  }
  
  # Compute the reward function
  f <- theta_0 + sum(theta * x) * p + tilde_theta * p^2
...
compute_optimal_price <- function(theta_0, theta, tilde_theta, x) {
  # Compute p^*(x)
  p_optimal <- - (t(theta) %*% x) / (2 * tilde_theta)
  
  # Ensure p^*(x) is within the bounds [0, 1]
  p_optimal <- max(0, min(1, p_optimal))
...
run_simulation <- function(n_P, n_Q,  gamma, kappa, C_r, r_star, tilde_p, theta_0, theta, tilde_theta, nu, some_revenue_function, C_I,   seed){
  ...
  if (n_P == 0){
    ...
    p <- generate_p_samples(n_P, tilde_p, r_star, kappa)
    Y_prob <- sapply(1:n_P, function(i) some_revenue_function(X[i, ], p[i], theta_0, theta, tilde_theta))
    Y <- runif(n_P , min = Y_prob - nu, max =  Y_prob + nu)
  ...
  p_optimal <- sapply(1:n_Q, function(i) compute_optimal_price(theta_0, theta, tilde_theta, X_target[i,]))
  f_optimal <- sapply(1:n_Q, function(i) some_revenue_function(X_target[i, ], p_optimal[i], theta_0, theta, tilde_theta))
  Y_optimal <- runif(n_Q , min =  f_optimal - nu, max =  f_optimal + nu)
  
  
  # Run the transfer learning pricing algorithm
  results_TLDP <- transfer_learning_pricing_se1(n_Q, X_target, D_P, C_r, kappa, d, gamma, theta_0, theta, tilde_theta, nu, C_I, some_revenue_function)
  
  results_ABE <- ABE_algorithm_se1(n_Q, X_target, d, theta_0, theta, tilde_theta, nu, some_revenue_function) 
  
  
  # Run the ExUCB algorithm
  results_ExUCB <- ExUCB_algorithm_se1(n_Q, X_target, d, theta_0, theta, tilde_theta, nu, some_revenue_function)
